<?php
 ...
 if(isset($_GET['vnp_Amount'])) {
   ...
   $code_cart = $_SESSION['code_cart_vnpay'] ?? ($_SESSION['$code_cart'] ?? '');
   if ($code_cart === '') {
      echo 'Khong tim thay ma don hang VNPAY trong phien dang nhap.';
      exit();
   }
   ...
   if (!isset($_SESSION['id_khachhang']) || !isset($_SESSION['cart']) || empty($_SESSION['cart'])) {
      echo 'Phien thanh toan da het han hoac gio hang khong hop le.';
      exit();
   }
   ...
   $cart_payment='MOMO';
    ...
    if (!$row_getvanchuyen || !isset($row_getvanchuyen["id_shipping"])) {
      echo 'Khong tim thay thong tin van chuyen.';
      exit();
    }
